...
(function ($, window, console, undefined) {
		...
		check: function () {
			...
			if (FooBox.browser.isSafari) {
				...
				if (FooBox.browser.iOS == true) {
					...
					FooBox.browser.version = /.*CPU.*OS 7_\d/i.test(navigator.userAgent) ? 7 : 0;
					if (FooBox.browser.version >= 7){
						FooBox.browser.css += ' fbx-safari-ios7';
					}
				}
	...
	FooBox.Modal = function (instance) {
		...
		this.show = function (first) {
			...
			var $body = $('body').addClass('fbx-active');
			...
			if (_this.FooBox.raise('foobox.beforeLoad', { 'item': item }).isDefaultPrevented()) { return; }
		...
		this.resize = function (size, success, error) {
			try {
				...
				var mt = -((size.height / 2) + padding + ((ibt + ibb) / 2) + (buttons ? mpb - mpt : 0)),
		...
		this.close = function () {
			if (!_this.FooBox.raise('foobox.beforeClose').isDefaultPrevented()){
				...
				var $body = $('body').removeClass('fbx-active');
	...
	FooBox.DeepLinking = function (instance) {
		...
		this.hash = {
			...
			set: function () {
				...
				window.location.replace(('' + window.location).split(FooBox.hashBang)[0] + FooBox.hashBang + hash);
			},
			/**
	...
	FooBox.Keyboard = function (instance) {
		...
		this.handlers = {
			...
			unbind: function(all){
				...
				_this.FooBox.element.unbind('foobox.afterShow', _this.handlers.afterShow);
			...
			initialized: function () {
				...
				if (_this.FooBox.options.keyboard.enabled == true){
					_this.FooBox.element.bind('foobox.afterShow', _this.handlers.afterShow);
					if (FooBox.isjQuery(_this.FooBox.modal.element)){
			...
			/**
			 * Handles the foobox.afterShow event allowing this addon to focus the modal to enable capturing of key events.
			 * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
			 */
			afterShow: function (e) {
				e.fb.modal.focus();
...
(function ($, FooBox) {
	...
	FooBox.Responsive = function (instance) {
		...
		this.handlers = {
			...
			unbind: function(all){
				...
				_this.FooBox.element.unbind({
					'foobox.beforeShow': _this.handlers.ios7beforeShow,
					'foobox.close': _this.handlers.ios7close
				});
			},
			/**
			 * Handles the foobox.initialized event binding the various events needed by this addon to function.
			...
			initialized: function () {
				...
				if (_this.FooBox.modal.element.hasClass('fbx-safari-ios7')){
			 ...
			 */
			ios7beforeShow: function(){
				_this.timers.ios7.start(_this.handlers.ios7, _this.FooBox.options.ios7interval);
			},
			/**
			 * For ios7 & Safari stop the periodic checks for the address and bookmarks bar when the modal is closed.
			 */
			ios7close: function(){
				_this.timers.ios7.stop();
			},
			/**
			...
			ios7: function(){
				...
				var o = window.orientation || 0,
					...
					landscape = o == 90 || o == -90,
					eh = (landscape ? screen.availWidth : screen.availHeight) * 0.85;

				if ((_ios7bars == null || _ios7bars == false) && ((landscape && h < eh) || (!landscape && h < eh))){
					_ios7bars = true;
					_this.ios7redraw();
				} else if ((_ios7bars == null || _ios7bars == true) && ((landscape && h > eh) || (!landscape && h > eh))) {
					_ios7bars = false;
					_this.ios7redraw();
				}
				_this.timers.ios7.start(_this.handlers.ios7, _this.FooBox.options.ios7interval);
	...
	FooBox.Responsive.metaCheck = function(){
		...
		if ($meta.length == 0){ // no tag, append it
			$('head').prepend('<meta name="viewport" content="width=device-width,initial-scale=1"/>');
		} else if ($meta.length > 0 && typeof content == 'string'){
			...
			if (!width){ parts.push('width=device-width'); }
			if (!initial_scale){ parts.push('initial-scale=1'); }
		...
		this.handlers = {
			...
			onCreateCaption: function (e) {
				...
				if (cont.hasClass('gallery')) {
					if (opt.captions.overrideTitle === false) { title = el.parents(".gallery-item:first").find(".wp-caption-text:first").html() || el.parents(".gallery-item:first").find(".gallery-caption:first").html() || ''; }
					if (opt.captions.overrideDesc === false) { desc = el.find('img').attr('alt') || ''; }
				} else if (cont.hasClass('wp-caption') || el.hasClass('wp-caption')) {
					if (opt.captions.overrideTitle === false) { title = el.find('img').attr('title') || el.parents(".wp-caption:first").find(".wp-caption-text:first").html() || ''; }
					if (opt.captions.overrideDesc === false) { desc = el.find('img').attr('alt') || ''; }
				} else if (el.parents(".wp-caption:first").length > 0) {
					if (opt.captions.overrideTitle === false) { title = el.parents(".wp-caption:first").find('img').attr('title') || el.parents(".wp-caption:first").find(".wp-caption-text:first").html() || ''; }
					if (opt.captions.overrideDesc === false) { desc = el.parents(".wp-caption:first").find('img').attr('alt') || ''; }
